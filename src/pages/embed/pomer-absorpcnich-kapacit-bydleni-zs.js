...
export default function EmbedPomerAbsorpcnichKapacitBydleniZs({ baseUrl }) {
  ...
  const { containerRef } = usePostMessageWithHeight(
    "paq_ua-kapacity-zari-2022_pomer-absorpcnich-kapacit-bydleni-zs"
  );
  ...
  const fillByOkresId = React.useMemo(() => {
    ...
    return capacitiesData.reduce((carry, okresCapacities) => {
      return {
        ...carry,
        [okresCapacities.id]: color(
          okresCapacities.pomer_absorpcnich_kapacit_bydleni_zs
        ),
      };
    }, {});
  }, [capacitiesData]);
  ...
  return (
    <div className={styles.container}>
      <Head>
        <title>Poměr absorpční kapacity bydlení a kapacity přes ZŠ</title>
      </Head>

      <main className={styles.container} ref={containerRef}>
        <h1>Poměr absorpční kapacity bydlení a kapacity přes ZŠ</h1>
              ...
              <div className={styles.tooltipContent}>
                ...
                <div className="main-value-line">
                  <strong>
                    {okresCapacitiesById[
                      okresId
                    ].pomer_absorpcnich_kapacit_bydleni_zs.toLocaleString(
                      "cs-CZ"
                    )}{" "}
                    %
                  </strong>{" "}
                  je poměr absorpční kapacity bydlení a kapacity přes ZŠ
                </div>
              </div>
            )}
          />
