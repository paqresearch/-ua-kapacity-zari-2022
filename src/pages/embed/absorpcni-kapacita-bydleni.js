...
export default function EmbedAbsorpcniKapacitaBydleni({ baseUrl }) {
  ...
  const { containerRef } = usePostMessageWithHeight(
    "paq_ua-kapacity-zari-2022_absorpcni-kapacita-bydleni"
  );
  ...
  const fillByOkresId = React.useMemo(() => {
    ...
    return capacitiesData.reduce((carry, okresCapacities) => {
      return {
        ...carry,
        [okresCapacities.id]: color(okresCapacities.bydleni_absorpcni_kapacita),
      };
    }, {});
  }, [capacitiesData]);
  ...
  return (
    <div className={styles.container}>
      <Head>
        <title>Absorbční kapacita regionu v bydlení — pozitivní varianta</title>
      </Head>

      <main className={styles.container} ref={containerRef}>
        <h1>Absorbční kapacita regionu v bydlení — pozitivní varianta</h1>
              ...
              <div className={styles.tooltipContent}>
                ...
                <div className="main-value-line">
                  <strong>
                    {okresCapacitiesById[
                      okresId
                    ].bydleni_absorpcni_kapacita.toLocaleString("cs-CZ")}
                  </strong>{" "}
                  uprchlíků lze v regionu ubytovat
                </div>
              </div>
            )}
          />
